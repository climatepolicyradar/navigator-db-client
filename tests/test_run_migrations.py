...
def test_run_migrations_with_pytest_mocker(mocker):
    ...
    mock_realpath = mocker.patch('db_client.run_migrations_script.os.path.realpath', return_value=test_path + "test.py")
    mock_dirname = mocker.patch('db_client.run_migrations_script.os.path.dirname', return_value=test_path)
    ...
    mock_Config().set_main_option.assert_called_once_with('script_location', test_path + '/alembic')
